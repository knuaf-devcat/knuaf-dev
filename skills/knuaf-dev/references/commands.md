...
- 도구가 없어 못 돌린 검사는 그 항목만 `blocked`·`not_run` 으로 남긴다. **학교 필수성은
  그대로다** — 도구가 없다는 이유로 요구사항이 사라지지 않는다.

## 능력 확인은 탐지가 아니라 시험이다

`doctor` 는 설치를 **탐지**할 뿐 실행 성공을 뜻하지 않는다. 파일 읽기·쓰기, DOCX 생성,
XLSX 생성, 재계산, 원문 추출, 글꼴, 렌더는 각각 작은 로컬 예제로 한 번 돌려 보고
실행 명령·결과·버전을 기록한다.
