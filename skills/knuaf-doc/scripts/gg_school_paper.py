...
- Body override (school_profile 필요):
  - body_markdown: str (optional, 정본 절을 합친 본문 마크다운.
    주면 아래 Ⅰ–Ⅵ 골격 대신 이 본문을 그대로 쓰고 앞머리만 생성한다.
    `Ⅰ. 머리말`로 시작해야 하며 빈 본문은 거절한다)
...
def custom_body(spec):
    """Return the caller's own body markdown, or None when none was supplied.

    이 생성기의 Ⅰ–Ⅵ 골격은 2020년 판 목차라, 2026년 강의자료 목차(Ⅰ머리말/
    Ⅱ농장현황/Ⅲ영농환경분석/Ⅳ농장비전/Ⅴ영농계획/Ⅵ맺음말/Ⅶ참고문헌)를 따라
    쓴 원고를 넣으면 장이 재배치된다. 그래서 정본 절을 합친 본문
    (`build/<개정>/review/검토용.md` 등)을 그대로 실을 길을 연다. 앞머리
    (겉표지·표제면·인준서·목차)는 생성기가 계속 붙이므로, 생성기를 우회해
    본문만으로 DOCX 를 만들 이유가 없어진다.

    말이 안 되는 본문은 조용히 통과시키지 않는다. 빈 본문으로 DOCX 가 나오면
    호출자는 그것을 "완료"로 읽는다.
    """

    body = spec.get("body_markdown")
    if body is None:
        return None
    ...
    # 시작하면 앞머리와 이어붙일 자리를 알 수 없다.
    first = body.splitlines()[0].lstrip("#").strip()
    if not first.startswith(BODY_MARKER):
        raise ValueError(
            "본문은 '%s'로 시작해야 함 (받은 첫 줄: %s)" % (BODY_MARKER, first[:40])
        )
    return body
...
def paper(spec):
    ...
    ``body_markdown`` replaces that generated body with the caller's own merged
    text.  The frontmatter is unchanged, so an empty advisor or submission date
    still stays ``[확인 필요]`` rather than being filled in.
    """

    body_markdown = custom_body(spec)
    profile = normalize_school_profile(spec)
    if not profile.get("enabled"):
        if body_markdown is not None:
            # 앞머리 없이 본문만 돌려주면 겉표지·인준서가 빠진 문서가 된다.
            raise ValueError("본문 직접 지정은 school_profile 이 있어야 함")
...
def main():
    ...
    ap.add_argument(
        "--body",
        help="정본 절을 합친 본문 마크다운 경로 (예: build/19/review/검토용.md). "
        "주면 Ⅰ–Ⅵ 골격 대신 이 본문을 쓰고 앞머리만 생성한다",
    )
    a = ap.parse_args()
    try:
        ...
        if a.body:
            spec["body_markdown"] = local(a.base, a.body).read_text(encoding="utf-8")
