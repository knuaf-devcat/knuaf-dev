...
import { COMPLETION, EMPTY, FIELD_HINT, FIELD_LABEL, INDEPENDENT_REVIEW_MISSING, NOT_APPROVAL_NOTE, NO_OVERWRITE_NOTE, describeError, officeHint, type DescribedError } from '../copy'
...
/**
 * The six script-backed tools, each inside a preset-keyed 펼침. Lives in 설정 > 고급 도구
 * (03-화면/08); failure feedback deep-links land on `data-preset="tools:<id>"`.
 */
export function OutputsTools() {
  ...
  return (
    <div>
      <Disclosure label="검토본보내기" preset="tools:export">
        <div className="card-head"><h2>검토본보내기</h2>{status && <Badge label={`${status.revision}번째 기록`} />}</div>
        <p>지금 정본의 본문을 <code>build/{status?.revision}/{kind}/</code>에 만들어요. {NO_OVERWRITE_NOTE}</p>
        <div className="row">
          ...
          <button className="primary" onClick={doExport}>만들기</button>
        </div>
        {/* SKILL.md:34 — the notice belongs to the moment of creating a submission candidate,
            before 만들기, not after. The button stays enabled: the warning is the gate. */}
        {kind === 'submission_candidate' && status?.lanes.content_review.independent_review_missing && (
          <div style={{ marginTop: 'var(--sp-3)' }}><Feedback kind="warning" title={INDEPENDENT_REVIEW_MISSING.title} body={INDEPENDENT_REVIEW_MISSING.body} /></div>
        )}
      ...
      <Disclosure label="논문 골격 만들기" preset="tools:paper">
      ...
      <Disclosure label="DOCX 만들기" preset="tools:docx">
      ...
      <Disclosure label="재무 엑셀" preset="tools:excel">
      ...
      <Disclosure label="Office 렌더" preset="tools:office">
      ...
      <Disclosure label="build 폴더" preset="tools:tree">
