...
export function Troubleshoot() {
  ...
  if (!root) return <div><Toolbar title="문제 해결" /><EmptyState icon="folder" title="먼저 홈에서 폴더를 여세요" /></div>
  ...
  const act = async () => {
    ...
    try {
      ...
      if (c.kind === 'restore' && c.rev != null && status) { const r = await rpc.restore(root, c.rev, status.revision); setMsg({ title: COMPLETION.restored, body: `개정 ${r.restored_from}의 정본을 개정 ${r.revision}으로 되돌렸어요. 복원 직전 상태는 ${r.pre_restore_snapshot}에 남아 있어요.` }) }
  ...
  return (
    <div>
      <Toolbar title="문제 해결" actions={<button onClick={load}><Icon name="refresh" size={16} /> 다시 진단</button>} />
      <p className="intro">{SCREEN_INTRO.troubleshoot}</p>
      ...
      {!hasProject && <Feedback kind="warning" title="이 폴더에는 아직 정본(project.json)이 없어요" body="보통 에이전트가 '시작하기'로 만들어요. 새 빈 폴더가 확실하면 여기서 만들 수도 있어요." actions={<button onClick={() => setConfirm({ kind: 'init' })}>빈 정본 만들기</button>} />}

      <section className="card">
        <div className="card-head"><h2>Python과 패키지</h2>{depsInfo && <StatusBadge status={depsInfo.ready ? 'pass' : 'blocked'} label={depsInfo.ready ? '준비됨' : '준비 필요'} />}</div>
      ...
      <section className="card">
        <div className="card-head"><h2>정본 스냅샷</h2><Badge label={`${Math.max(hist.length - 1, 0)}개`} /></div>
        <p className="caption">저장할 때마다 직전 정본이 <code>migration/revision-N.json</code>에 남아요. 복원은 새 개정으로 기록되고 절 파일·원문·build 폴더는 그대로예요.</p>
        {hist.length <= 1 ? <div className="caption">{EMPTY.snapshots.title}</div> : (
          <div className="list">
            {hist.map((h) => (
              <div key={h.revision} className="item">
                <div className="head"><span><strong>개정 {h.revision}</strong>{h.current ? <span className="caption"> · 현재</span> : null} <code className="muted">{h.request_id ?? ''}</code></span>
                  <span className="row">{h.current ? null : h.snapshot_ok ? <StatusBadge status="pass" label="해시 일치" /> : <StatusBadge status="fail" label="없음/불일치" />}{!h.current && h.snapshot_ok && <button onClick={() => setConfirm({ kind: 'restore', rev: h.revision })}>이 개정으로 복원</button>}</span></div>
              </div>
            ))}
          </div>
        )}
      ...
      <Confirm open={confirm?.kind === 'unlock'} title="잠금 해제" body={<p>같은 기기에서 이미 종료된 작업의 잠금만 해제해요. 다른 창에서 에이전트가 이 폴더에 저장 중이 아닌지 확인하셨나요?</p>} confirmLabel="해제" danger onConfirm={act} onCancel={() => setConfirm(null)} />
      <Confirm open={confirm?.kind === 'restore'} title={`개정 ${confirm?.rev}로 복원`} body={<p><b>현재 상태를 먼저 저장해요.</b> 그다음 정본만 개정 {confirm?.rev} 시점으로 되돌리고 새 개정 번호를 붙여요. 절 파일과 발행물은 그대로예요.</p>} confirmLabel="복원" danger onConfirm={act} onCancel={() => setConfirm(null)} />
